...
HELP_ITEMS = (
    ...
    ("single-order-access", "單筆查閱與列印授權", ("root",), (
        "進入訂單，點查閱與列印授權，選合作車行人員並載入設定；分別勾選查看、列印，填原因後儲存或撤銷。",
        "這是原有範圍以外的額外授權；不改銷售來源、佣金、金額或文件公司，不授予編輯、財務或補助。帳號仍需相應的查詢及列印功能權限。",
        "人員轉換身分／車行後舊授權失效，即使轉回也需重新授權。撤銷額外授權不移除原本由本人／本車行範圍取得的存取權。")),
