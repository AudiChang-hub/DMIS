...
- 工作日曆：查看人事行政總處每月同步的國定假日，也可人工補登公司例外休假日；官方同步失敗時會沿用既有資料。
- 品牌牌險：公式、固定總額或人工輸入。
...
已儲存或已交付的訂單：進入訂單明細，在上方展開「調整台數與傭金歸屬」，選車行、填原因，再按一次「儲存歸屬」。不需要解鎖整張訂單；選擇「原銷售車行：○○」可恢復原歸屬。取消不會儲存。

- 例如 B 賣的一台指定給 A：這台計入 A 的台數，基礎傭金與台數獎金都給 A；B 其他訂單不受影響。
- 原訂單仍記錄 B 是實際銷售車行，基礎傭金與車行加減額沿用原訂單金額，不改套 A 的基礎費率。
