...
install -m 0644 \
    ...
    "$SYSTEMD_DIR/dmis-next-backup.service"
install -m 0644 \
    "$PROJECT_DIR/deployment/systemd/dmis-next-backup.timer" \
    "$SYSTEMD_DIR/dmis-next-backup.timer"
systemctl daemon-reload
systemctl enable --now dmis-next-backup.timer
systemctl status dmis-next-backup.timer --no-pager
