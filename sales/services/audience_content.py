...
RELEASE_RULES = {
    ("1.17.2", "改善"): [("order_start",), ("screen:order_finance", "internal")],
    ("1.17.1", "改善"): [("root",)],
    ("1.17.0", "新增"): [("root",), ("root",), ("root",)],
    ("1.16.2", "修正"): [("root",)],
    ("1.16.1", "改善"): [("order_list",)],
    ("1.16.0", "新增"): [("order_start", "internal"), ("root",), ("order_list",), ("root",)],
    ("1.15.1", "修正"): [("order_list", "internal")],
    ("1.15.0", "新增"): [("root",), ("order_list",), ("order_documents_print",)],
    ("1.15.0", "修正"): [("order_start",)],
    ...
    ("1.12.0", "新增"): [("root",), ("order_start",), ("screen:order_finance", "internal")],
    ("1.12.0", "改善"): [("screen:order_finance", "internal"), ("order_list", "internal"), ("order_list", "internal")],
...
HELP_ITEMS = (
    ("assisted-order", "代合作車行開單", ("order_start", "internal"), (
        "建立訂單的來源選代合作車行開單，選擇實際銷售車行，再確認公司名稱、地址、電話及統編。缺少或過期的公司資料需 admin 先完成設定。",
        "實際操作人仍保留為建立人；銷售來源與客戶文件公司為確認的車行。對方具有本車行訂單範圍即可查看代開單，具有文件列印權限即可在待接單時列印。")),
    ("single-order-access", "單筆查閱與列印授權", ("root",), (
        "到資料維護區的帳號與權限，選人員後以帳號資料、畫面與操作、訂單範圍、指定訂單四個分頁管理。訂單的管理本單權限入口也會帶到同一工作區。",
        "指定訂單先搜尋單號或車主，再選沿用一般設定或本筆自訂。自訂時勾選允許、未勾選禁止；填寫原因並儲存後立即生效。畫面上方顯示目前實際權限。帳號功能未開放時需先啟用，不會偷偷擴權。",
        "恢復一般設定可能重新開放本人／本車行訂單的原有權限。舊版額外授權在修改前維持原效果；人員轉換身分／車行後，舊設定失效，即使轉回也需重新確認。不改來源、公司、金額或帳務。")),
    ...
    ("order-attachments", "下訂附件與其他配件", ("order_start",), (
        "自然人需提供證件正反面；存摺封面選填，選檔後立即預覽，儲存後才完成上傳。新舊車主同一人預設不勾，勾選後不必重複上傳舊車主證件及存摺。",
        "配件清單找不到時，具調價權限的人員選其他，再填名稱、數量、售價及工資；不會新增為全站配件主檔。切換贈送會將售價與工資歸零。",
        "已存附件在訂單的車主資料區，點縮圖放大或按下載檔案；不支援預覽的格式仍可下載。")),
    ...
    ("order-workspace-finance", "整理一張訂單的收入與支出", ("screen:order_finance", "internal"), (
        ...
        "分期外客戶應收以保存的收款明細為準，畫面、訂購單與交付檢查採相同金額。分期還款總額是每期乘期數，並非客戶需再付店內的尾款或分期公司撥款。",
    ...
    ("custom-discount", "實際費用與總價折扣", ("screen:order_finance",), (
        ...
        "在金額收支資訊的內部折扣核准，具財務編輯及核定權限者可輸入任意折數（例如 9.25）或折讓金額，按儲存並套用折扣，不必跳頁申請。",
        "折扣以未扣訂金及舊車折抵的總價計算，不改分期公司撥款、佣金與成本。若他人已更新訂單，系統拒絕覆寫，請重新核對。",
        "金額收支資訊改用收款明細列表，預設僅訂金；按新增收款，自訂項目、金額、日期與收款方式，勾選已確認後才列入實收。",
        "新增款項請選客戶收款或分期公司撥款，兩類分開加總，不以名稱猜測。附件、備註及刷卡手續費在各列更多資料；應收設定保留原金額與調整原因，不是實收。",
        "新分期單的客戶應收仍是牌險加配件；訂金不重複計入。分次收款依已確認金額合計，分期公司撥款不能抵客戶欠款；取消確認或移除後重新計算。",
        "調整應收請填寫原因；車輛成本可依財務授權手動調整，保存後不被自動價表覆寫。交付頁不再重複輸入收款。")),
    ...
    ("search-orders", "訂單進度與查詢", ("order_list",), (
        "使用姓名、訂單編號或車型搜尋，選擇狀態縮小清單。本人／本車行範圍由 admin 設定；本車行範圍包含馭盛代開的訂單，另可查看 admin 額外授權的單筆訂單。舊 Excel 未記錄建立者的訂單不會自動歸戶。",
        "訂單接單後顯示接單人與時間；重新整理可取得最新狀態。")),
    ("customer-documents", "列印客戶簽署文件", ("order_documents_print",), (
        "訂單成立頁可直接列印本筆訂購單與全部客戶簽署文件；不必先開放訂單查詢，也不會跳到其他客戶資料。",
        "待接單即可依本人／本車行或單筆文件授權列印，不代表馭盛已接單。不包含內部成本、佣金、收支帳務或補助作業資料。開單公司未確認時，須先請 admin 處理。")),
    ("gift-accessories", "贈送配件", ("screen:order_gift",), (
        "贈送與售價調整由 admin 分別授權。選贈送時售價與工資自動歸零，不能藉此修改其他費用；原已存贈品不因撤權變成加購。",)),
    ...
    ("account-management", "帳號與分類權限", ("root",), (
        "資料維護區 → 帳號與權限，是 admin 的統一管理入口；選人員後管理帳號資料、畫面與操作、訂單範圍及指定訂單。合作車行與訂單頁只保留前往同一中心的連結。",
