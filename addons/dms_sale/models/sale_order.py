...
class SaleOrder(models.Model):
    ...
    @api.model
    def _normalize_import_match_text(self, value):
        return (value or '').strip()

    @api.model
    def _normalize_import_match_code(self, value):
        return ''.join(ch for ch in (value or '').upper() if ch.isalnum())

    @api.model
    def _find_cross_source_import_order(self, vals, incoming_origin=None):
        customer_name = self._normalize_import_match_text(vals.get('customer_name'))
        if not customer_name:
            return self.browse()

        id_number = self._normalize_import_match_code(vals.get('id_number'))
        customer_phone = self._normalize_import_match_code(vals.get('customer_phone'))
        if not id_number and not customer_phone:
            return self.browse()

        domain = [
            ('customer_name', '=', customer_name),
            ('sale_origin', 'in', ['excel', 'order_processor']),
            ('active', '=', True),
            ('state', '!=', 'cancel'),
        ]
        if incoming_origin:
            domain.append(('sale_origin', '!=', incoming_origin))

        candidates = self.search(domain, order='create_date desc, id desc')
        if id_number:
            candidates = candidates.filtered(
                lambda order: self._normalize_import_match_code(order.id_number) == id_number
            )
        else:
            candidates = candidates.filtered(
                lambda order: self._normalize_import_match_code(order.customer_phone) == customer_phone
            )

        incoming_registration_date = vals.get('registration_date')
        if incoming_registration_date:
            incoming_registration_date = fields.Date.to_date(incoming_registration_date)
        incoming_product_id = vals.get('product_id') or False
        incoming_dealer_id = vals.get('dealer_id') or False
        incoming_source_product_name = self._normalize_import_match_text(vals.get('source_product_name'))
        incoming_excel_sync_id = self._normalize_import_match_text(vals.get('excel_sync_id'))
        incoming_source_folder = self._normalize_import_match_text(vals.get('source_folder'))

        ranked = []
        for order in candidates:
            if (incoming_excel_sync_id and order.excel_sync_id
                    and order.excel_sync_id != incoming_excel_sync_id):
                continue
            if (incoming_source_folder and order.source_folder
                    and order.source_folder != incoming_source_folder):
                continue

            signals = 0
            complements = 0

            order_phone = self._normalize_import_match_code(order.customer_phone)
            if customer_phone:
                if order_phone and order_phone != customer_phone:
                    continue
                if order_phone == customer_phone:
                    signals += 1

            order_registration_date = order.registration_date
            if incoming_registration_date and order_registration_date:
                if order_registration_date != incoming_registration_date:
                    continue
                signals += 2
            elif incoming_registration_date and not order_registration_date:
                order_create_date = order.create_date.date() if order.create_date else False
                if (order_create_date
                        and abs((order_create_date - incoming_registration_date).days) <= 45):
                    complements += 1

            if incoming_product_id:
                if order.product_id and order.product_id.id != incoming_product_id:
                    continue
                if order.product_id and order.product_id.id == incoming_product_id:
                    signals += 2
                elif not order.product_id:
                    complements += 1
            elif incoming_source_product_name:
                order_source_product_name = self._normalize_import_match_text(
                    order.source_product_name)
                if order_source_product_name and order_source_product_name != incoming_source_product_name:
                    continue
                if order_source_product_name == incoming_source_product_name:
                    signals += 1

            if incoming_dealer_id:
                if order.dealer_id and order.dealer_id.id != incoming_dealer_id:
                    continue
                if order.dealer_id and order.dealer_id.id == incoming_dealer_id:
                    signals += 1
                elif not order.dealer_id:
                    complements += 1

            if signals >= 2 or (signals >= 1 and complements >= 1):
                ranked.append((
                    signals,
                    complements,
                    order.create_date or datetime.min,
                    order.id,
                    order,
                ))
    ...
    @api.model
    def _prepare_import_update_vals(self, existing_order, vals):
        write_vals = dict(vals)
        incoming_origin = write_vals.get('sale_origin')
        if (incoming_origin and existing_order.sale_origin
                and incoming_origin != existing_order.sale_origin):
            write_vals.pop('sale_origin', None)
        return write_vals
