...
- 找不到來源名稱時，不要亂選；可由管理者到「通路與傭金」新增。歷史 Excel 匯入時也能在原畫面直接新增分類與來源。
...
1. 從「資料維護區 → 每月車行價目表」選品牌與月份。
2. 系統會帶入該品牌啟用中的車型、車色、目前售價與分期方案；可選擇沿用上一期贈品、通路與排序。
3. 直接點價目表成品中的文字或金額，調整建議售價、現金優惠、現金價、各期月付、顧客贈品、通路與備註；手機點欄位時會開啟放大編輯區。
4. 分期金額格內的「設定」可補分期公司與開辦費；灰色操作提示不會印出。畫面就是列印成品，不必切換預覽反覆核對。
5. 按「儲存並列印」可保存後直接開啟列印；按「發布價目表」後，該版本會鎖定並依生效日同步售價與分期版本。
6. 已發布內容若要修正，請按「建立修訂版」，不要直接改掉舊版。

品牌 Logo 可在品牌資料維護；單月若有活動版 Logo，可在該月價目表上傳覆蓋。發布後會保存當時的 Logo 與品牌名稱，之後主檔異動不會改變舊版列印內容。

> 「顧客贈品」是給車行看的活動說明，不會自動建立成某張訂單的贈送配件；實際成交仍需在訂單確認。

### 其他共用資料

- 配件：售價、工資與參考成本。
- 通路與傭金：可維護本店員工、合作車行、網路平台及自訂分類；只有系統處理方式為「合作車行」者套用車行傭金。「查看年節送禮名單」只切換清單，「管理送禮名單」可搜尋並批次勾選，儲存後會以完整勾選結果為準。
- 分期公司：名稱、客服電話。
- 工作日曆：國定假日。
- 品牌牌險：公式、固定總額或人工輸入。
