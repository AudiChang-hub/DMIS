...
class ChannelFinanceTests(TestCase):
    ...
    def test_sales_source_quick_toggles_preserve_expanded_region_and_scroll_state(self):
        ...
        self.assertContains(response, 'data-source-list-control="holiday-gift"')
        self.assertContains(response, 'data-allow-multiple-submit="true"', count=2)
        ...
        self.assertContains(response, "data-source-region-template")
        self.assertContains(response, "hydrateRegion(group)")
        self.assertContains(response, 'data-source-list-control="holiday-gift"] button[role="switch"]')
        self.assertContains(response, 'form.dataset.submitting = "false";')
        self.assertContains(response, 'data-source-list-control="active"')
        self.assertContains(
            response,
