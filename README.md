...
1.11.0 訂單三頁籤工作區：訂單資訊、金額收支資訊、補助申請資訊同頁操作，切換保留輸入、明確儲存、原地更新摘要與衝突保護。接待下單及既有財務計算不變。規格見 `specs/043-order-workspace/`。

1.10.0 新版簡報改善：獨立下單調價授權、自訂分期、配件贈送及合計、指定車行圖文公告、送禮預設名單與活動刪除、平順頁籤、解鎖下載及文案搜尋。需求見 `specs/042-site-review-followup/`。新調價權限不自動開放給既有人員；由 admin 在內部帳號的「建立訂單」分類或車行帳號勾選，與成本／佣金／淨利權限分離。

1.9.0 全站操作改善：公告分眾與歷史、手動年節送禮、admin 說明文字管理、接待配件及自訂總價折扣。需求與驗收追蹤見 `specs/041-site-review/`。正式部署通過健康驗證後，以 `record_release` 記錄首次成功發布時間（重跑不覆寫）；舊版本未知時間不回填。
...
- 選車入口與展示管理統一品牌／車型／型號／能源別篩選；詳細頁直接顯示已選色，選現金或分期方案後於頁底接續填單。簽署選擇連同本人草稿保存，送出重新驗證有效車色、售價與方案，失效不得靜默換價。

- 展示管理只列啟用車款，支援品牌／車型／型號連動篩選；僅編輯啟用車色，各色集中管理圖片，未上架照片僅 admin 可預覽。
- 展示圖片選檔後即時預覽並標示尚未儲存，可取消恢復原圖；按「儲存展示設定」才上傳，支援 JPEG／PNG／WebP、每張最多 8 MB。

- 主導覽依序為首頁、建立訂單、全部訂單、營運總表、報表中心、資料維護區；建立採完整接待流程，成功只顯示本筆結果，不自動打開客戶列表或內部財務。建立與查詢分開授權。
- 正式訂單刪除由一般人員說明原因提出申請，可自行取消；admin 審核或駁回。admin 可明確確認刪除已完成及關聯訂單，處理庫存與獎金分配，保留收款、附件及稽核，不把刪除當作實際退款或退車。

- 系統選車入口 `/catalog/`：上架車款圖片、規格、有效售價與分期，登入後沿用共用下單。
- 合作車行的「登入帳號」由 admin 管理多人帳號及下單／唯讀模式，與帳號總覽同步。
- 車款預設不公開；管理者須備妥有權使用的圖片並明確上架。原 Google Sites／Forms 不自動切換或停用。
- 車款以下拉選單篩選；各車色可分別上傳圖片，選色卡寬版四欄、窄版兩欄，選擇後帶入共用下單。
- 選車列表逐色顯示「年式/型號/型式/顏色」，缺圖仍保留；機種與售價及年式抽屜直接列出啟用色名。既有主圖可由 admin 明確對應至單一車色。
- 車行的選車、訂單進度及下單可分別設定；權限分類支援全選／全部取消，未授權的空分類隱藏。
- 淨利預設鎖定，admin 於「全部訂單 → 查看淨利」獨立授權；以本人密碼解鎖目前 session 5 分鐘，匯出另需勾選匯出權限。財務編輯權限另計。
- 版本歷程與使用說明依帳號可用功能過濾；新增發布項目須同步 `sales/services/audience_content.py` 的受眾規則。

- 系統首頁提供版本與公告；營運總表集中經營趨勢、收款風險與工作量，明細統一至全部訂單。
- admin 由資料維護區管理公告、置頂、預約發布與下架；修訂紀錄及版本衝突保護。
...
- admin 可於「帳號與權限 → 畫面權限」逐人設定查看、操作與匯出，並逐份授權報表；先預覽再確認，保留異動與還原歷程。
