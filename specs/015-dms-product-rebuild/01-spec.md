...
- 價目版本需支援批次加入多個 SKU，一次建立多筆價格基準列
- 價目版本批次加入 SKU 時，需可先輸入統一的現金價與牌價，再套用到所有新建價格列
...
- 以 List 為主
- SKU 不提供獨立頂層或次層入口，僅於產品模板表單中的「產品項」頁籤維護
- 產品項頁籤需同時顯示啟用與停用資料，避免停用後無法重新啟用
- 產品項頁籤需提供列級「複製」動作，便於同模板下快速建立多色或相近版本 SKU
- 產品項列級複製時，系統需自動清空舊 `internal_code` 並重新生成唯一代碼，避免前台沿用舊值造成誤判重複
- 產品項列級複製完成後，應只重新整理當前模板畫面，不得重複推入相同 breadcrumb
- 產品項頁籤刪除列時，對於未被其他交易資料引用的產品項，應直接刪除該 SKU，而非僅解除模板關聯
- 模板清單中的 `SKU 數量` 僅計算啟用中的產品項，停用項不計入數量
- 產品模板需提供可見的「複製」動作，讓內部人員可快速複製既有模板與其產品項
- 涉及 SKU 選取的下拉與搜尋結果，需明確顯示 `出廠年份` 與 `顏色`
- 主要畫面不得依賴圖片
- 圖片若保留，僅作相容欄位或次要資訊
- 不得干擾既有車行管理 UI
