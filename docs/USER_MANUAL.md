...
## 全站操作更新（1.9.0，2026/09/17）

- 首頁「公告與更新」切換最新消息／版本歷程；版本預覽最近兩次，完整歷程另有分頁。時間使用台北時區。
- admin 在「資料維護區 → 系統公告管理」選所有人、店內、車行或搜尋勾選指定人員。到期自動隱藏，歷史列表可封存／刪除並保留紀錄。
- 「資料維護區 → 年節送禮」手動建立活動與名單，記錄完成、取消完成及封存；不會自動產生每月名單。
- 「資料維護區 → 說明文字管理」由 admin 編輯頁面／欄位提示及訂購合約確認事項，可還原預設；文字不改變計算或權限。列印欄位限制字數以維持版面。
- 接待下單保留已授權主選單，可選擇加購配件；財務欄位仍只在內部管理畫面開放。
- 訂單作業的總價優惠可自行輸入折數或減少金額，核准後取代舊優惠。分期公司撥款、佣金與成本不跟著打折；申請後總價變更須重新申請。
- 內部財務人員可附原因調整配件售價、工資及牌險明細，系統試算保留供比對；已結算帳務仍需依既有更正流程。
- 證件正反面在 A4 直式頂端左右排列並保留用途浮水印；下載不會鎖住原頁按鈕。

## 選車與付款方案（1.8.0）

1. 建立訂單入口依「品牌 → 車型 → 型號 → 能源別」篩選；手機與電腦順序相同。展示管理也提供相同篩選。
2. 點車色卡的「查看車款與分期」，詳細頁直接顯示該色。只有需要更換時才展開「更換車色」。
3. 在售價與分期選現金，或分期公司／期數，核對車價、每期金額及開辦費，再於頁面最下方按「確認選擇，接續填單」。
4. 登入後保留選擇；填單頁先顯示摘要並收合已選車款與付款段落。資料會連同本人草稿儲存，重開不用重選。
5. 更換時按「更換車款／付款方案」，在同頁調整，不會清空車主資料。若原方案停用、到期或價格異動，系統會阻擋送出並要求重新確認，不會靜默套用其他方案。
6. 接待入口不顯示內部撥款、成本、佣金或淨利；無建立權限仍不可下單。

## 訂單刪除與還原

- 以以下 1.7.0 流程為準：一般人員提出申請，admin 才能確認刪除；編輯訂單權限不包含刪除申請。
- 回收區保留歷史證據及還原入口；不實體刪除附件或偽造退款。還原含關聯訂單須由 admin 檢查庫存衝突。

## 接待下單與刪除審核（1.7.0，取代舊入口及刪除限制）

- 主選單：首頁 → 建立訂單 → 全部訂單 → 營運總表 → 報表中心 → 資料維護區。手機與首頁常用功能的建立訂單都進同一入口。
- 建立訂單直接選車、選色、填資料，店內與車行同流程。接待畫面不呈現其他客戶訂單、成本、佣金與淨利。送出只顯示本筆成立及待接單結果，可再建立一筆。
- 接待頁上方與其他頁面一致，依序為「首頁 → 建立訂單」。按「首頁」或左上系統名稱即可回首頁；成立結果頁也有「回首頁」按鈕。需要內部財務或後續作業時，再從首頁進入已授權管理畫面。建立、查詢與財務分開授權；沒有權限不顯示入口，直接網址亦受保護。
- 我的接待草稿只列本人草稿，可加入常用功能。面對客戶前先開啟本次草稿，不在接待頁列其他草稿。
- 一般內部人員須有刪除申請操作授權，填寫原因後送 admin 審核，顯示「刪除確認中」。原交易狀態與統計在核准前不變；申請人可取消尚未處理的申請。
- admin 從全部訂單 → 刪除審核查看、駁回或確認。已完成及有關聯訂單可以由 admin 明確勾選作廢確認後刪除，不必轉移到新單。
- 核准後移至回收區，退出查詢、報表、統計及收款待辦；保留歷史與收款證據，不代表實際退款或退車。解除配車時，已領牌／已交付車輛改為停用待核對，不當成可售新車。
- 獎金分配保留作廢紀錄，結算台數與預計額重算，帳面合計扣除原單分配，其餘訂單不轉嫁。含關聯的還原只限 admin；車輛已另行使用時整筆拒絕，避免破壞新交易。
- Excel 寫入前再檢查交易鍵，包含已刪除的來源快照；刪除不會讓相同交易重新匯入。

## 選車列表與啟用顏色（1.5.0）

- 選車列表以「年式/型號/型式/顏色」識別每個選項；所有啟用色都會顯示，缺少照片不影響選擇。統計區分車型數與車色選項數。
- 點選卡片會在車款詳細頁預選該色，再按接續填單；不同車款、停用或無效的車色不會被預選。
- 機種與售價的「啟用顏色」直接列出色名，年式抽屜也分別列出，不混用不同年份的顏色。
- admin 可在展示管理將既有主圖明確對應至一個啟用車色；其他顏色不會共用照片。已有不同照片時不能直接覆蓋，請改用該色上傳欄位。

## 展示管理與啟用車色（1.6.0）

- 展示管理僅列啟用車款，未上架仍可編輯；停用資料與照片保留，不刪除。
- 「品牌 → 車型 → 型號」下拉連動，變更上層會清除下層選擇，按「套用篩選」更新結果。可搭配關鍵字，換頁保留條件，清除篩選返回全部啟用車款。
- 編輯頁每個啟用車色獨立區塊，集中預覽、上傳及移除；無啟用色時提示至機種資料設定。未選新檔不改原圖，同色不可同時上傳與移除。
- 未上架圖片僅 admin 可預覽，公開圖片入口仍拒絕讀取。停用車型的舊編輯連結不開放；儲存途中若車型／車色停用或圖片更新，會阻擋儲存並要求重新整理。
- 「車款主圖與既有圖片對應（進階）」保留既有主圖功能，不會把主圖自動套用所有車色。

### 圖片即時預覽（1.6.1）

- 各車色與進階主圖選檔後立即預覽，並標示「尚未儲存」；按「儲存展示設定」才會上傳。
- 「取消此次圖片變更」清空新選檔並恢復已儲存圖片；原本沒有圖片則恢復待補。勾選移除會清除該色新選檔，重新選圖會取消移除勾選，均須儲存才生效。
- 每張最多 8 MB，支援 JPEG／PNG／WebP；空檔、格式不符或無法讀取會顯示錯誤並阻擋送出，請重選或取消此次變更。
- 重新整理會清除尚未儲存的選檔；若其他欄位未通過伺服器驗證，瀏覽器基於安全性不會保留選檔，請修正後重新選擇圖片。

## 選車、車色及人員功能（1.4.0）

- 主選單 → 建立訂單，先用車款下拉選單選車，再點選該機種的車色圖片。寬版四欄、窄版兩欄；接續填單會保留機種與顏色。未送出前仍是草稿。
- admin 在「資料維護區 → 選車展示管理」逐色維護圖片；缺圖顯示顏色名稱與「圖片待補」，不以其他顏色的圖片代替。圖片須有使用權，每張上限 8 MB。
- 合作車行 → 登入帳號，管理該車行每一位人員。登入帳號可自訂或按「使用建議帳號」；建議為車行代碼＋人員序號，不會更動舊帳號。
- 選車入口、訂單進度、下單可分別勾選。分類全選／全部取消只改該大項；未開放任何子項就不顯示大項。實際權限以儲存後為準。
- 內部人員在「帳號與權限 → 畫面權限」按分類調整，先預覽再確認。只有 admin 可管理畫面授權。
- 版本歷程與線上使用說明只顯示目前帳號有權使用的內容；本手冊保留完整管理說明，勿當成對外車行說明文件。

## 查看淨利（1.4.0）

1. admin 在人員畫面權限的「全部訂單 → 查看淨利」勾選查看；需要下載含淨利資料時另外勾選匯出。
2. 全部訂單預設鎖定淨利；按「輸入密碼查看」，輸入目前登入者本人的密碼。admin 也需要驗證。
3. 目前登入工作階段開放 5 分鐘，翻頁不延長，可按立即鎖定。到期遮蔽淨利，但不重載頁面或清掉未儲存表單。
4. 未解鎖不能依淨利排序；含淨利的營運匯出、舊資料財務預覽及原始財務搜尋也受保護。一般統計報表不會因此整頁鎖定。
5. 此機制不收回已下載、截圖或已顯示的副本；原始收入與成本的編輯權限另計，不等於限制所有財務資料。請勿在一般備註手動填寫不應公開的淨利。

## 首頁版本歷程

### 自訂常用功能

1. 首頁按「編輯常用功能」，依分類勾選功能，也可搜尋名稱或切換「只看已勾選」。
2. 在「首頁顯示預覽」按上移、下移或移除，排列自己常用的工作入口。
3. 恢復預設、全部取消勾選都只改本次草稿；按「儲存常用功能」才生效。取消則不儲存。
4. 同一帳號在不同裝置共用首頁收藏，不影響別人，也不覆蓋手機「更多 → 快速前往」設定。
5. 可以儲存空清單。功能被撤權後會自動隱藏，需要新增權限請聯絡 admin。
6. 另一視窗已先儲存時會提示衝突並載入最新設定，確認後再存，不會偷偷覆蓋。

### 查閱版本更新

首頁顯示目前正式版號、完整發布日期與新增／改善／修正內容。最新版本預設展開，
點選版本標題可展開或收合。正式編版前的近期更新另列為「提交日期」，不是發布日期。
「技術資訊」保留建置識別碼，右上角「檢查更新」功能不變。
日常公告由 admin 維護；正式版本歷程隨程式發布，不由公告頁編輯。

## 報表視覺化設計（2026-09-10）

### 報表列表快速分頁

來源訂單、人口分組、圖表下鑽明細與彙總表的多頁清單提供「第一頁／上一頁／下一頁／最後一頁」。可輸入頁碼按「前往」或 Enter；翻頁保留篩選、排序及圖表選取，下鑽明細仍在側欄更新。手機按鈕分成兩列。彙總表切換每頁筆數回到第一頁，僅切換目前已載入資料，不改變 Top N 或匯出範圍。

### 報表分類設定（B 方案）

admin 可由「資料維護區 → 管理者工具 → 報表分類設定」管理車種類別、型號規則與未分類清單。先儲存草稿，再按「試算草稿影響」，核對型號、分類與訂單數，勾選影響確認後發布。正式報表及 CSV 才會改用新版本；原始訂單與財務不變。

精確型號優先於文字開頭／系列；比對區分大小寫，不需輸入 regex。數字系列例如 UQ125DA，數字／-代碼系列例如 GSX-R150。不同類別的系列規則重疊會被阻擋，可改用精確型號例外。未分類清單依草稿顯示，每頁可勾選多個型號，指定類別後建立精確規則。

類別可改名、配色、調整管理清單順序或停用；高辨識配色仍依設計器設定覆寫。已有規則或曾發布使用的類別不能硬刪除；停用後其規則失效，資料歸入保留類別「其他」。類別移轉請修改規則的目標類別後試算。版本還原只建立新草稿，須重新試算與發布。多視窗版本衝突或試算後資料變動時，請重載／重新試算，不會覆寫其他人的修改。

2026-09-11：銷售／分析頁統一以「車行／平台」篩選報表名稱資料，不再並列主檔 ID 篩選。讀者的原報表能源控制項已移除，報表固定能源範圍仍有效。來源明細及 CSV 使用「售出車行、能源別、收款確認、收款價」，不再顯示重複實收欄；售出車行空白顯示馭盛。已收款／未收款沿用原確認旗標，不代表全額結清；沒有收支資料仍顯示待補收支資料。車種分類將 UQ／UC／UG／UT 數字型號修正為速克達、GSX／DS 系列歸為擋車，其他待辨識車型不任意歸類。
...
1. **每天先看系統首頁**：確認帳號、版本更新與公告；經營分析和工作量請進營運總表。
...
```text
電腦上方：系統首頁｜全部訂單｜營運總表｜資料維護｜使用說明｜檢查更新｜登出
...
系統首頁提供版本、公告與常用入口，不顯示財務與客戶清單。營運總表集中十二月走勢、同期比較、收款風險與工作量，不重複列出訂單；全部訂單查單與逐筆處理；報表中心分析車型與通路。

- 本月新成立訂單依成立日期，領牌成交台數、車款成交額及淨利依領牌日期；只計截至今日，排除取消／退款及取消待退款。
- 本月至今比較上月同期，短月截至月底；月底比較完整上月。前期零顯示「無比較基準」，不是增加 100%。
- 車款成交額＝車款售價合計，不等於實際收款。淨利沿既有公式，只合計可計入資料，不包含公司租金、薪資等費用；圖表斷線或破折號代表沒有可計入資料，不是零獲利。
- 月份數值表可查看精確數字及淨利可計入筆數，按核對明細進入相同日期條件的全部訂單。只有有權限的人能開啟明細。
- 待收款＝收款明細應收大於已收；已有收款待確認＝已有實收登記但營運收款未確認。風險含所有月份目前未結事項，各類可能重疊。取消待退款仍須處理全額退訂金。

### 訂單排序、狀態與取消

- 表頭直接點擊追加排序，再點切換方向；不需 Shift。日期顯示新→舊／舊→新、金額大→小／小→大、文字依系統正序／反序。
- 排序條件列提供逐項「移除」「提前」「延後」，不影響其他條件；「全部清除」才清掉所有排序。變更後回第一頁，保留搜尋、狀態與每頁筆數。
- 領牌日期是第一順位時：遞減未領牌在最前、遞增在最後。車主和訂單編號共用欄位，但可各自排序。
- 狀態色塊同時保留文字：淺灰＝草稿、深石墨底白字加 × 符號＝已取消／已退款、黃橙＝待配／待調、藍＝進行中、綠＝完成、紅橙＝退款或補件提醒。自家車行來源顯示「馭盛」。
- 取消／退款訂單缺日期時顯示「—」，不再提示未領牌或待補領牌日。淨利顯示「—」且不計成交彙總，包含取消明細時亦同；原收退款、費用、日期及異動紀錄不清除。
- 外觀含深淺色及高對比模式；可用 Tab 辨識焦點。狀態不只靠顏色辨認。
...
- 入口：「資料維護區 → 系統公告管理」或首頁的「管理系統公告」。
- 新增標題、純文字內容、置頂及台北時間起訖；結束可留白。
- 新公告「儲存草稿」不會公開；「儲存並發布」會在到達開始時間後顯示，結束時間到達自動隱藏。
- 編輯時「儲存」保持原發布狀態；「取消發布」可隱藏內容並保留修訂紀錄。
- 其他視窗已修改時會阻擋覆寫，請重新載入。一般人員不能透過網址或送出表單管理公告。
- 公告對所有已登入人員可見，不放客戶個資、密碼或財務機密。版本說明隨部署更新，日常公告不用重新部署。
- 原「戰情」授權保留為營運總表全公司指標；一般首頁不需要財務權限。訂單、庫存、操作、匯出仍分開驗權。
...
全部訂單點入後，以「訂單資訊／金額收支資訊／補助申請資訊」切換；不重新整理，也不自動儲存金額。

- 訂單資訊：車主、車款與需求；配車、領牌與交付，頁尾可展開處理紀錄。
- 金額收支資訊：查閱收款、成本與各項收入支出；編輯訂單可調整成交條件。成交條件與實際收支有各自儲存按鈕。
- 補助申請資訊：舊車資料、申請項目、送件進度、文件與匯款追蹤；申請金額不等於已入帳。
- 頁籤圓點表示尚未儲存，切換時會保留輸入；离開前提醒。版本或欄位衝突時不覆寫，請保留修改內容並核對最新資料。
- admin 授權與淨利密碼保護不變；無財務權限不提供財務頁籤。建立訂單的接待流程不變。
