...
class DmsProduct(models.Model):
    ...
    max_hp = fields.Integer(string="最大馬力 (hp)")
    max_torque = fields.Integer(string="最大扭力 (Nm)")

    # ── 動力規格（電車） ──────────────────────────────────
    power_system = fields.Char(string="動力系統")
    max_output = fields.Float(string="最大功率 (kW)")
    ev_max_hp = fields.Integer(string="最大馬力 EV (hp)")
    ev_max_torque = fields.Integer(string="最大扭力 EV (Nm)")
    ev_efficiency = fields.Char(string="能源效率 EV (km/kWh)")
