...
點選圖表分類會直接連動同頁其他圖表與資料表，不會自動擠出訂單明細。再點同一分類取消；勾選「複選分類」或按住 Ctrl／⌘ 點擊可增加或移除分類。同一圖的多選代表「任一符合」，不同圖的選取則需同時符合。每圖最多 20 個分類。
...
本頁銷售來源、領牌年月等複選欄會自動套用，不必再按按鈕；日期等手動輸入仍可用「套用篩選」。選單未勾選代表不限；「清除我的篩選」會清除個人查看條件，但不解除報表固定條件。
...
套用篩選與圖表點選會在頁內更新，瀏覽器上一頁可返回先前條件。更新失敗或逾時保留上次成功畫面，可按「重試更新」；資料版本已更動時請重新整理。原 DataStudio 部分嵌入圖表無法顯示，尚不能視為所有原站互動與數字均已等效驗收。

admin 由「資料維護區 → 管理者工具 → 報表設計管理」編輯。一般查看使用「報表中心」；既有報表查看權限不因新版設計器而改變。

「總車輛銷售」使用銷售總覽版型：主要條件為銷售來源、領牌年月；左右並排堆疊座標圖與占比圖，分類數字可展開查看。共用分類說明收在頁首「資料口徑與說明」，不影響固定條件。管理者可在報表設定的「閱讀版型」選擇一般或銷售總覽，其他報表不會因此一起變動。

1. 中央畫布顯示 DMIS 實際查詢結果，點圖表標題可在右側修改分類、指標及圖表種類。
2. 拖曳標題左側把手排序；右下角把手調整半寬／全寬與高度。也可使用右側上移、下移、寬度及高度欄位；尺寸把手支援方向鍵。
3. 資料設定修改後，按「更新實際圖表」核對；查詢失敗時保留輸入，不會寫入訂單或發布內容。
4. 按「儲存並發布」一次保存並更新報表中心；只想暫存可用「儲存草稿」。歷史還原只還原草稿，核對後須再次發布。
5. 整份報表的名稱、固定範圍及附表設定收在畫布上方。每份最多 8 張圖，手機自動單欄；目前為網格編排，不是任意重疊的自由座標畫布。
