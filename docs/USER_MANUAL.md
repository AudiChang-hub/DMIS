...
- admin 可直接使用；其他內部人員須在「帳號與權限 → 全部訂單 → 刪除與還原訂單」另授予查看及操作權限，編輯訂單權限不包含刪除。
- 全部訂單或訂單明細按「刪除訂單」，核對訂單、填原因並勾選確認。訂單移至回收區，一般列表、搜尋、統計及車行入口不再顯示，原始附件與操作紀錄仍保留。
- 從全部訂單上方「已刪除訂單／還原」搜尋，再填寫原因並確認還原。原狀態、編號和金額恢復顯示，不重算歷史財務。
- 刪除不是取消交易或退款。有配車、領牌、交付、實收未退、待退款或結算關聯時，確認頁會列出原因，必須先處理業務更正。系統不會自動刪除金流或釋放庫存。
...
- 全部訂單 → 建立訂單，先用車款下拉選單選車，再點選該機種的車色圖片。寬版四欄、窄版兩欄；接續填單會保留機種與顏色。未送出前仍是草稿。
