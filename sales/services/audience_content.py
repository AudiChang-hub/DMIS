...
HELP_ITEMS = (
    ...
    ("order-attachments", "下訂附件與臨時配件", ("order_start",), (
        "自然人需提供證件正反面；存摺封面選填。是否有汰舊選是後，可另附舊車主證件及存摺，不必等文件齊全才先建立訂單。",
        "配件清單找不到時，具調價權限的人員可填臨時配件名稱、數量、售價及工資；不會新增為全站配件主檔。",
        "圖片及 PDF 可先看縮圖並點擊放大；其他文件格式以檔名下載卡提供。")),
