...
### 4.3 產品模板表單（`view_product_template_form`）— 單頁式佈局（無 Tab）

```
[ 品牌 ] [ 機種 ] [ 型式 ] [ 型號 ] [ 能源 ] [ 啟用 ] [ 備註（inline）]

─── 產品項 ─────────────────────────────────────────────────────────────
  [⬡] [⎘]  內部代碼  年份  車色  36期分期價  現金直扣  現金價  18期專案  顧客贈品
            ← 全部可直接在列表內修改 →
            ← 牌價 / 活動價 / 活動說明 / 啟用 : optional 欄位（預設隱藏）→
```

- 移除 `<notebook>` / `<page>` 結構，`備註` 欄位改放在主 group 右側
- `[⬡]` 按鈕（原「開啟」）：僅用於進入顏色與分期方案管理
- `[⎘]` 按鈕（複製）：保留原有行為
- 新增可直接編輯欄位：`36期分期價`、`現金直扣`、`18期專案`、`顧客贈品`（來自 spec 020）

### 4.4 產品項 Dialog（`view_product_form_installment_dialog`）— 單頁式佈局（無 Tab）

```
[內部代碼]   [年份]  [啟用]  [模板]

─── 顏色 ───────────────────────────
  名稱 │ 啟用   ← inline editable

─── 分期方案 ────────────────────────
  分期公司 │ 期數 │ 計算基準 │ 年利率 │ 設定費 │ 開辦費 │ 月付金(計算) │ 備註 │ 異動說明
  ← inline editable →
```
...
- 價格基準不提供獨立選單，統一於價目版本表單中的「價格基準」頁籤維護
