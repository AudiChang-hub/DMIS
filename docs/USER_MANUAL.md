...
限 `admin`：在錯誤列按「修正並補匯」，於既有訂單差異區按「原買家退訂？核對並換買家補匯」。這一步只開啟預覽，不立即更動。

1. 核對原買家、新買家、原訂單編號與車輛識別號碼。先儲存本列暫存修正，其他頁未儲存的內容不會帶入。
...
4. 新買家仍待領牌／交車時，選「尚待領牌／交車」，並依合約填成交車價與應收尾款總額；系統保留預計日期、配車及庫存預留，不標記完成。新買家本列現金／刷卡實收記入新單尾款，原買家付款不轉入。其後回到一般訂單核對分期、牌險與費用並完成領牌／交付。已完成的歷史交易才選「已完成領牌及交車」，不可填未來領牌日期。
5. 輸入原單編號及原因後送出。系統一次完成原單退訂、解除占車、新買家補匯；補匯失敗則全部回復。原單、Excel、收支與更正前後歷程保留，不把原款項轉給新買家。

預覽超過 15 分鐘或核對期間資料改變，請重新載入核對，不能套用舊資料。相同買家只改領牌日期不屬於退訂換買家，請核對原訂單更正。
