    ...
    def test_tabs_media_and_responsive_layout(self):
        ...
        with tempfile.TemporaryDirectory() as media, override_settings(MEDIA_ROOT=media):
            ...
            with sync_playwright() as p:
                ...
                discount = page.locator('[data-workspace-save="discount"]')
                discount.locator('[name="mode"]').select_option('amount')
                discount.locator('[name="amount"]').fill('900')
                reason = discount.locator('[name="reason"]')
                reason.fill('')
                expect(reason).to_have_id('discount_request_reason')
                expect(page.locator('#discount_request_reason')).to_have_count(1)
                submissions = []
                page.on('request', lambda request: submissions.append(request.url) if request.method == 'POST' and 'discount' in request.url else None)
                discount.locator('button[type="submit"]').click()
                summary = discount.locator('[data-form-error-summary]')
                expect(summary).to_contain_text('請填寫「申請原因」。')
                expect(summary).not_to_contain_text('取消原因')
                summary.get_by_role('button', name='請填寫「申請原因」。').click()
                expect(reason).to_be_focused()
                discount.locator('label[for="discount_request_reason"]').click()
                expect(reason).to_be_focused()
                self.assertFalse(submissions)
                discount.locator('[name="amount"]').fill('')
