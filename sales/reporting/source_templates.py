...
def total_vehicle_sales():
    """p_oe0r8mk2wd 的四圖與明細；資料口徑與待核對事項另列說明。"""
    ...
    return {
        "title": "總車輛銷售", "audience": "admin", "date_basis": "registration_date",
        "description": "依原報表四圖與明細建立。資料讀取 DMIS，包含未填領牌日期；原始來源與 DMIS 筆數仍有差異，尚未完成逐筆對帳與全部互動驗收。",
...
def electric_platform_sales():
    ...
    config.update(title="電動車－網路平台銷售統計", page_order=30,
...
def gasoline_platform_sales():
    ...
    config.update(title="油車－網路平台銷售統計", page_order=70,
...
def dealer_sales(gasoline=False):
    ...
    config.update(title=vehicle + "－車行銷售統計", page_order=80 if gasoline else 40, reader_layout='dealer_overview')
...
def dealer_counts(gasoline=False):
    ...
    vehicle = "油車" if gasoline else "電動車"
    config.update(title=vehicle + "－台數統計", page_order=90 if gasoline else 50,
...
def model_analysis(kind):
    ...
    return {
        "title": title, "audience": "admin", "date_basis": "registration_date",
...
def age_sex_analysis():
    return {
        "title": "性別 X 年齡", "audience": "admin", "date_basis": "registration_date",
...
def sex_model_color_analysis():
    ...
    config.update(title="性別 X 車型顏色", page_order=130, include_records=True,
