...
7. 在 P20 dashboard 新增 `P20-4 車行區域銷量排行（長條圖）`，直接呈現熱門地區。
8. 將 P20 dashboard 篩選器調整為 `領牌年月`、`銷售來源`、`能源類型`、`車行縣市`、`車行區域`、`車行名稱` 六個下拉式欄位。
9. 將 `領牌年月` 預設為腳本套用時計算的最近半年（含當前月份），並將 `銷售來源` 預設為 `車行`。
