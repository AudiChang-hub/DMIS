...
def section_overview(doc):
    ...
    add_kv_table(doc, [
        ...
        ("(3) 產品主檔（位於『車銷管理』下）", "產品頁面（模板）、價格表（SKU 完整清單）；分期規則模板 / 費用類型 / 規則掛接目前為後台資料表，無菜單入口"),
...
SECTIONS = [
    ...
    {
        "title": "3. 產品主檔（DMS Product，選單位於「車銷管理」下）",
        "intro": "產品主檔為 015 重建後的独立模組 dms_product，但選單項目統一收納在「車銷管理」選單下，依序為「產品頁面」、「價格表」、「車輛銷售」、「車款銷售分析」、「同步紀錄」、「Excel 銷貨匯入」。車銷與佣金依賴本模組提供的模板、車色、這個世代的價格資料。",
        "items": [
            {"h": "3.1 產品頁面（模板，Product Template）",
             "brief": "以「品牌 + 機種（family_name） + 型式 + 型號 + 能源型式」為主鍵的車型主檔。表單上半部填主檔（都是必填欄位加「顧客贈品」說明）；下半部「產品項」為 inline tree，可直接從表單新增同一模板下不同年份 / 車色的 SKU，設定建議售價、現金直扣、現金價與各期分期價。2026 年起提供「複製此產品項」按鈕（最左側 fa-copy），複製後年份會被清空，使用者填新年份儲存即可，加速「年份重製」作業。tree 中最左側另提供「開啟詳細資料」外部連結按鈕（fa-external-link），可跳轉到單筆 dms.product 的 form 完整編輯。",
             "steps": ["進入「車銷管理 → 產品頁面」；清單可以品牌 / 機種 / 型式 / 型號 查詢，可以「品牌」或「能源型式」分組。",
                       "點「新增」：依畫面提示填入「品牌 / 機種 / 型式 / 型號 / 能源型式」與選填「顧客贈品」，儲存。",
                       "「產品項」表格「加入資料行」輸入年份 / 車色 / 各價格欄位；複製舊車色可點資料行最左侧 fa-copy 按鈕。",
                       "詳細編輯（含不在 inline tree 的欄位）請點資料行最左侧 fa-external-link 跳至詳細 form。"],
             "shots": [("20_template_list", "產品頁面清單（車銷管理 → 產品頁面）"), ("20_template_form", "產品頁面表單（含下方 inline 「產品項」）")]},
            {"h": "3.2 價格表（SKU 完整清單）",
             "brief": "價格表以 dms.product（SKU）為單位顯示「品牌 / 機種 / 型式 / 型號 / 年份 / 能源型式 / 車色 / 建議售價 / 現金直扣 / 現金價 / 12 ~ 60 期分期價 / 顧客贈品 / 附加費用說明 / 啟用」。\n\nPhase 10 UI 調整重點：「機種」欄 sticky 凍結、標題列凍結；表格改為不換行、以橫向捲動避免揚壓；標題字體 12px；顧客贈品 / 附加費用說明 / 車色以「pre-line」完整換行顯示，不被裁切。最左侧提供「開啟詳細資料」外部連結按鈕，點選可跳到單筆產品的 form 畫面；inline 編輯仍不受影響；品牌 m2o 下拉不再被相鄰 sticky 欄遮擋。",
             "steps": ["進入「車銷管理 → 價格表」檢視所有 SKU；可使用上方搜尋列以品牌 / 機種 / 型號 篩選。",
                       "inline 修改資料：點任一資料欄輸入內容後，点上方「儲存」按鈕即可。",
                       "詳細修改：點資料列最左侧的 fa-external-link 進入 form 畫面，適合大量欄位同時調整。",
                       "新增產品：請到「車銷管理 → 產品頁面」建立模板，並在其下「產品項」表格加入 SKU。"],
             "shots": [("21_pricetable_list", "價格表（sticky 機種欄、fa-external-link 「開啟詳細資料」按鈕）"), ("21_pricetable_form", "價格表詳細 form 畫面")]},
            {"h": "3.3 分期規則模板 / 費用類型 / 規則掛接（現階段為後台資料表，尚無選單入口）",
             "brief": "「分期規則模板」（dms.installment.rule）設定期數、首期月份、利率、最低順期；「費用類型」（dms.fee.type）為車銷單可選的附加費用項目；「規則掛接」（dms.installment.rule.binding）串接費用類型與分期規則模板到某個產品項。\n\n這三個資料表現階段未提供前台選單入口，只能透過「設定 → 技術 → 動作」或直接以 URL `action=145 / 146 / 147` 進入。下一個階段才會考慮在「車銷管理」或「設定」下佈置選單。",
             "steps": ["以「設定 → 技術 → 動作 → 視窗」查找 `action_installment_rule` / `action_fee_type` / `action_installment_rule_binding`，點「執行」進入列表。",
                       "或以 URL 直接進入：http://localhost:8069/web#action=145（分期規則模板）、action=146（費用類型）、action=147（規則掛接）。",
                       "規則掛接需同時選「產品項」、「費用類型」與「分期規則模板」，並可勾選「只適用某些品牌 / 車型」限縮適用範圍。"],
             "shots": [("26_installment_rule", "分期規則模板清單"), ("27_fee_type", "費用類型清單"), ("28_rule_binding", "規則掛接清單")]},
        ],
    },
    {
        "title": "4. 車銷管理（DMS Sale）",
        "intro": "車銷管理選單同時收納產品主檔與銷售單據；本章僅說明「車輛銷售」、「同步紀錄」與「Excel 銷貨匯入」。產品主檔（產品頁面 / 價格表 / 分期規則模板等）請見「3. 產品主檔」。",
        "items": [
    ...
    },
    {
        ...
        "items": [
            ...
            {"h": "5.3 P4 電動車銷售統計",
             ...
             "steps": ["於集合中點選「P4 電動車銷售統計」。",
                       "可依時間、車型、通路切片，瞭解電車各車型佔比與走勢。"],
             "shots": [("metabase_p3_ev_sales", "P4 電動車銷售統計")]},
            {"h": "5.4 P18 油車銷售統計",
             "brief": "以油車為主的銷售儀表板，提供月別、車種、車型等銷售結構。",
             "steps": ["於集合中點選「P18 油車銷售統計」。"],
             "shots": [("metabase_p9_fuel_sales", "P18 油車銷售統計")]},
            {"h": "5.5 P20 油車-車行銷售統計",
             "brief": "以「合作車行」為主軸的油車銷售排行與貢獻度，便於評估車行表現。",
             "steps": ["於集合中點選「P20 油車-車行銷售統計」。",
                       "可使用搜尋列鎖定特定車行或時段檢視。"],
             "shots": [("metabase_p11_dealer_sales", "P20 油車-車行銷售統計")]},
            {"h": "5.6 其他內建儀表板",
             "brief": "系統另內建以下儀表板，可於 Metabase 集合中直接點開使用："
                      "P2 銷售機種及車型統計、P3 通路銷售統計、P5 / P6 / P7 / P8 電動車系列、"
                      "P9 / P10 / P11 / P12 電動車客群分析、P13 / P14 / P15 / P16 / P17 基隆公益系列、"
                      "P19 油車網路平台銷售、P21 油車佣金明細、P22 油車台數統計等。",
